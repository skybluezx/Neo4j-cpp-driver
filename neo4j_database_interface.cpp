...
#include "neo4j_database_interface.hpp"

#include <sstream>

#include "../util/code/base64.hpp"
#include "../exception/database_exception.hpp"

#include "database_interface_kit.hpp"

namespace databaseInterface
{
    Neo4jDatabaseInterface::Neo4jDatabaseInterface(std::shared_ptr<databaseDriver::Neo4j> database)
        :database(database), headers(NULL), curl(NULL), responseHeaderString(""), responseString("")
    {
        
    };
    
    Neo4jDatabaseInterface::Neo4jDatabaseInterface(std::shared_ptr<databaseDriver::Neo4j> database, std::string host, std::string port, std::string user, std::string password)
    ...
    std::string Neo4jDatabaseInterface::getHost()
    {
        return this->host;
    };
    
    std::string Neo4jDatabaseInterface::getPort()
    {
        return this->port;
    };
    
    std::string Neo4jDatabaseInterface::getUser()
    {
        return this->user;
    };
    
    std::string Neo4jDatabaseInterface::getPassword()
    {
        return this->password;
    };
    
    void Neo4jDatabaseInterface::connectDatabase()
    {
        ...
        util::Base64::Encode(userAndPasswordString, &userAndPasswordEncodeString);
        ...
        if (this->curl)
        {
            ...
            //指定接收返回协议头的回调方法
            curl_easy_setopt(this->curl, CURLOPT_HEADERFUNCTION, Neo4jDatabaseInterface::responseHeaderHandeler);
            //指定接收返回协议头的存储位置
            curl_easy_setopt(this->curl, CURLOPT_HEADERDATA, &this->responseHeaderString);
            //指定接收数据的回调方法
            curl_easy_setopt(this->curl, CURLOPT_WRITEFUNCTION, Neo4jDatabaseInterface::responseHandeler);
            //指定接收数据的存储位置
        ...
        {
            //curl_easy初始化失败，抛出异常
            exception::DatabaseException exception(2);
            throw exception;
        }
    };
    
    void Neo4jDatabaseInterface::closeDatabase()
    {
        this->responseHeaderString = "";
        this->responseString = "";
    ...
    Json::Value Neo4jDatabaseInterface::cypherQuery(std::string cypher, Json::Value properties)
    {
        if (this->curl)
        {
            ...
            {
                //post请求执行失败，抛出异常
                throw exception::DatabaseException(6, this->getHost(), this->getPort(),  this->getUser(), this->getPassword(), "");
            }
            else
            {
                //对返回数据的协议状态进行检查
        ...
        {
            exception::DatabaseException exception(3, this->getHost(), this->getPort(),  this->getUser(), this->getPassword(), "");
            throw exception;
        }
    };
    
    Json::Value Neo4jDatabaseInterface::createNode(const Json::Value &properties, const std::string &label)
    {
        if (this->curl)
        {
    ...
    Json::Value Neo4jDatabaseInterface::createNode(const Json::Value &properties, const std::vector<std::string> &labels)
    {
        return this->createNode(properties, Neo4jDatabaseInterfaceKit::getLabelString(labels));
    };
    
    bool Neo4jDatabaseInterface::deleteNode(unsigned long long int nodeID)
    {
        if (this->curl)
        {
    ...
    bool Neo4jDatabaseInterface::deleteNodeAndAllRelationshipsOfTheNode(unsigned long long int nodeID)
    {
        /*
    ...
    unsigned long long int Neo4jDatabaseInterface::deleteNodeByLabelAndProperties(std::string label, Json::Value &properties)
    {
        std::string cypher = "";
        std::string whereString = Neo4jDatabaseInterfaceKit::getWhereString("data", properties);
    ...
    unsigned long long int Neo4jDatabaseInterface::deleteNodeByLabelsAndProperties(std::vector<std::string> &labels, Json::Value &properties)
    {
        return this->deleteNodeByLabelAndProperties(Neo4jDatabaseInterfaceKit::getLabelString(labels), properties);
    };
    
    bool Neo4jDatabaseInterface::getNode(unsigned long long int nodeID, Json::Value &node)
    {
        if (this->curl)
        {
            std::stringstream sstream;
    ...
    Json::Value Neo4jDatabaseInterface::selectNodesByLabelAndProperties(std::string label, Json::Value &properties)
    {
        if (this->curl)
        {
            std::string httpUrlForTransaction = "http://" + this->host + ":" + this->port + "/db/data/transaction/commit";
    ...
    Json::Value Neo4jDatabaseInterface::selectNodesByLabelsAndProperties(std::vector<std::string> &labels, Json::Value &properties)
    {
        return this->selectNodesByLabelAndProperties(Neo4jDatabaseInterfaceKit::getLabelString(labels), properties);
    };
    
    Json::Value Neo4jDatabaseInterface::selectNodesByAnotherLinkedNode(std::string selectedLabel, Json::Value &selectedProperties, std::string relationType, Json::Value &relationProperties, std::string linkedLabel, Json::Value &linkedProperties)
    {
        //初始化cypher语句为空
        std::string cypher = "";
        ...
        if (selectedProperties != Json::nullValue)
        {
            ...
            selectedWhereString = Neo4jDatabaseInterfaceKit::getWhereString("n", "n_", selectedProperties, "_id");
        }
        ...
        if (relationProperties != Json::nullValue)
        {
            ...
            relationWhereString = Neo4jDatabaseInterfaceKit::getWhereString("r", "r_", relationProperties, "_id");
        }
        ...
        if (linkedProperties != Json::nullValue)
        {
            ...
            linkedWhereString = Neo4jDatabaseInterfaceKit::getWhereString("m", "m_", linkedProperties, "_id");
        }
    ...
    Json::Value Neo4jDatabaseInterface::selectNodesByAnotherLinkedNode(std::string selectedLabel, Json::Value &selectedProperties, std::string relationType, Json::Value &relationProperties, unsigned long long int linkNodeID)
    {
    ...
    bool Neo4jDatabaseInterface::updateNodeByID(unsigned long long int nodeID, Json::Value &properties)
    {
        std::string cypher = "START data=node({_id}) SET " + Neo4jDatabaseInterfaceKit::getSetString("data", properties) + " RETURN data";
    ...
    bool Neo4jDatabaseInterface::replaceNodeByID(unsigned long long int nodeID, Json::Value &properties)
    {
        if (this->curl)
        {
            std::stringstream sstream;
    ...
    bool Neo4jDatabaseInterface::insertRelationship(unsigned long long int rootNodeID, unsigned long long int otherNodeID, std::string typeName, Json::Value &properties, Json::Value &relationship)
    {
        if (this->curl)
        {
            std::stringstream sstream;
    ...
    bool Neo4jDatabaseInterface::insertRelationship(unsigned long long int rootNodeID, unsigned long long int otherNodeID, std::string typeName, Json::Value &relationship)
    {
        Json::Value properties;
        return Neo4jDatabaseInterface::insertRelationship(rootNodeID, otherNodeID, typeName, properties, relationship);
    };
    
    bool Neo4jDatabaseInterface::insertRelationship(unsigned long long int rootNodeID, unsigned long long int otherNodeID, std::string typeName)
    {
        Json::Value properties;
    ...
    bool Neo4jDatabaseInterface::deleteRelationship(unsigned long long int relationshipID)
    {
        if (this->curl)
        {
            std::stringstream sstream;
    ...
    unsigned long long int Neo4jDatabaseInterface::deleteRelationshipsOfOneNode(unsigned long long int nodeID, std::string type)
    {
    ...
    unsigned long long int Neo4jDatabaseInterface::deleteAllRelationshipsOfOneNode(unsigned long long int nodeID)
    {
        return this->deleteRelationshipsOfOneNode(nodeID, "all");
    };
    
    unsigned long long int Neo4jDatabaseInterface::deleteAllOutgoingRelationshipsOfOneNode(unsigned long long int nodeID)
    {
        return this->deleteRelationshipsOfOneNode(nodeID, "out");
    };
    
    unsigned long long int Neo4jDatabaseInterface::deleteAllIncomingRelationshipsOfOneNode(unsigned long long int nodeID)
    {
        return this->deleteRelationshipsOfOneNode(nodeID, "in");
    };
    
    bool Neo4jDatabaseInterface::getRelationship(unsigned long long int relationshipID, Json::Value &relationship)
    {
        if (this->curl)
        {
            std::stringstream sstream;
            ...
            else
            {
                ...
                if (statusCode == "200")
                {
                    ...
                    relationship["_id"] = metaData["id"];
                    relationship["_type"] = metaData["type"];
                    relationship["_start"] = Neo4jDatabaseInterfaceKit::getNodeOrRelationshipID(responseJson["start"].asString());
                    relationship["_end"] = Neo4jDatabaseInterfaceKit::getNodeOrRelationshipID(responseJson["end"].asString());
    ...
    Json::Value Neo4jDatabaseInterface::getRelationshipsOfOneNode(unsigned long long int nodeID, std::string type)
    {
        if (this->curl)
        {
            ...
            else
            {
                ...
                if (statusCode == "200")
                {
                    ...
                    for (int i = 0; i < relationshipInfoList.size(); i++)
                    {
                        ...
                        relationship["_id"] = metadata["id"];
                        relationship["_type"] = metadata["type"];
                        relationship["_start"] = Neo4jDatabaseInterfaceKit::getNodeOrRelationshipID(relationshipInfo["start"].asString());
                        relationship["_end"] = Neo4jDatabaseInterfaceKit::getNodeOrRelationshipID(relationshipInfo["end"].asString());
    ...
    Json::Value Neo4jDatabaseInterface::getAllRelationshipsOfOneNode(unsigned long long int nodeID)
    {
        return this->getRelationshipsOfOneNode(nodeID, "all");
    };
    
    Json::Value Neo4jDatabaseInterface::getAllOutgoingRelationshipsOfOneNode(unsigned long long int nodeID)
    {
        return this->getRelationshipsOfOneNode(nodeID, "out");
    };
    
    Json::Value Neo4jDatabaseInterface::getAllIncomingRelationshipsOfOneNode(unsigned long long int nodeID)
    {
        return this->getRelationshipsOfOneNode(nodeID, "in");
    };
    
//    Json::Value Neo4jDatabaseInterface::selectRelationshipsByTypeAndProperties(std::string type, Json::Value &properties)
//    {
//        if (this->curl)
//        {
//            std::string httpUrlForTransaction = "http://" + this->host + ":" + this->port + "/db/data/transaction/commit";
...
//    };
    ...
    Json::Value Neo4jDatabaseInterface::getRelationsBetweenTwoNodes(unsigned long long int fromNodeID, unsigned long long int toNodeID, std::string type)
    {
        std::stringstream sstream;
    ...
    bool Neo4jDatabaseInterface::replaceRelationshipProperties(unsigned long long int relationshipID, Json::Value &properties)
    {
        if (this->curl)
        {
            std::stringstream sstream;
    ...
    Neo4jDatabaseInterface::~Neo4jDatabaseInterface()
    {
        //清理headers和curl
        if (this->headers != NULL)
        {
    ...
    size_t Neo4jDatabaseInterface::responseHeaderHandeler(char *buffer, size_t size, size_t nitems, void *userdata)
    {
        //计算本次收到的数据长度
    ...
    size_t Neo4jDatabaseInterface::responseHandeler(char *ptr, size_t size, size_t nmemb, void *userdata)
    {
        //计算本次收到的数据长度
